...
import {
    ...
    NTodo,
    NToggle
} from '@components/Post/notion';
import { Divider } from '@components/ui';
import { BlockType, NotionChildrenType } from '@types';

import NImage from './notion/image/NImage';
    ...
    const render = () => {
        switch (type) {
            case BlockType.PARAGRAPH:
                if (block.paragraph)
                    return <NParagraph paragraph={block.paragraph} />;
                break;
            case BlockType.HEADING_1:
                if (block.heading_1)
                    return <NHeading1 head1={block.heading_1} />;
                break;
            case BlockType.HEADING_2:
                if (block.heading_2)
                    return <NHeading2 head2={block.heading_2} />;
                break;
            case BlockType.HEADING_3:
                if (block.heading_3)
                    return <NHeading3 head3={block.heading_3} />;
                break;
            case BlockType.QUOTE:
                if (block.quote)
                    return (
                        <NQuote quote={block.quote} children={block.children} />
                    );
                break;
            ...
            case BlockType.TODO:
                if (block.to_do)
                    return <NTodo todo={block.to_do} blockId={block.id} />;
                break;
            ...
            case BlockType.BOOKMARK:
                if (block.bookmark)
                    return <NBookmark bookmark={block.bookmark} />;
                break;
            case BlockType.CODE:
            ...
            case BlockType.IMAGE:
                if (block.image) {
                    return <NImage imageBlock={block} />;
                }
                break;
